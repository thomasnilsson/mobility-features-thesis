...
class Features {
  ...
  Features(this._date, this._uniqueDates, this._data, this._stops, this._places,
    ...
    this._placesDaily = _places
        ...
        .toList();
    this._movesDaily =
        _moves.where((d) => d.stopFrom.arrival.zeroTime == _date).toList();
  }
  ...
  double get routineIndex {
    ...
    /// Extract past dates
    List<DateTime> datesHist = _uniqueDates.where((d) => d.isBefore(_date.zeroTime)).toList();

    /// TODO: Only calculate error between a start and end time
    for (DateTime d in datesHist) {
      List<Stop> histStops = stops.where((s) => s.arrival.zeroTime == d.zeroTime).toList();
  ...
  Place get _homePlace {
    ...
    for (DateTime d in _uniqueDates) {
      List<Stop> stopsOnDate =
          stops.where((s) => s.arrival.zeroTime == d).toList();
